...
fn significantDigits(v: f32, digits: usize) f32 {
    ...
    //warn("significantDigits: v={} vabs={}", v, vabs);
    //defer warn("\n");
    ...
    //warn(" df={}", df);
    var sig: f32 = math.floor(math.round(v * mf / df));
    //warn(" sig={}", sig);
    return sig;
}
...
pub fn approxEq(l: f32, r: f32, digits: usize) bool {
    ...
    var l_sig = significantDigits(l, digits);
    var r_sig = significantDigits(r, digits);
    var diff = math.fabs(r_sig - l_sig);
    result = diff == 0;
    // warn(" l_sig={} r_sig={} diff={}", l_sig, r_sig, diff);
    return result;
}
